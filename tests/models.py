...
class TestProductModel(unittest.TestCase):
    ...
    def test_update_a_product_empty_ID(self):
        """It should Update a Product but give empty ID"""
        product = ProductFactory()
        product.id = None
        product.create()
        self.assertIsNotNone(product.id)
    ...
    def test_find_by_price(self):
        """It should Find Products by price_value"""
        products = ProductFactory.create_batch(10)
        for product in products:
            product.create()
        price_value = products[0].price
        count = len([product for product in products if product.price == price_value])
